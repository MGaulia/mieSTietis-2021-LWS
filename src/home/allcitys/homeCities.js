...
const HomeCities = (props) =>{
    ...
    const renderLabs = () =>{
        console.log("asdasdsdxxxx",labels)
        if(labels==null){ 
            return
        }
        let row = labels.map((label)=>{
            return <div style={{marginRight:"1px", fontSize:"10px", width:"50px",}}>{label}</div>
        })
        return(
            <div style={{paddingLeft:"70px", display:"flex", flexDirection:"row"}}>
                {row}
            </div>
        )
    }
    ...
    display:"flex",flexDirection: "column"}}>
        ...
        <Barchartas thisCity={props.thisCity} group={"šiukšles"}></Barchartas>
...
const Barchartas = (props) => {
    ...
    const Icon = () => {
        const style1 = {position:"absolute", marginLeft:"20px", marginTop:"0px"}
        const style2 = {position:"absolute", marginLeft:"250px", fontSize:"12px"}
        switch (true) {
            ...
            case (props.group==="transportas"):
                return(<div>
                    <span style={style1}>
                    <svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" fill="currentColor" class="bi bi-minecart" viewBox="0 0 16 16">
